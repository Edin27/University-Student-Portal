    ...
    public void contactStaff() {
        ...
        if (sharedContext.currentUser instanceof AuthenticatedUser) {
            ...
            if (!inquirerEmail.matches("^[a-zA-Z0-9_+&*-]+(?:\\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,7}$")) {
                ...
                Log.AddLog(sharedContext.getCurrentUserEmail(), Log.ActionName.CONTACT_STAFF, inquirerEmail, Log.Status.FAILURE);
                return;
            }
        }
        ...
        if (subject.strip().isBlank()) {
            ...
            Log.AddLog(sharedContext.getCurrentUserEmail(), Log.ActionName.CONTACT_STAFF, subject, Log.Status.FAILURE);
            return;
        }
        ...
        if (text.strip().isBlank()) {
            ...
            Log.AddLog(sharedContext.getCurrentUserEmail(), Log.ActionName.CONTACT_STAFF, text, Log.Status.FAILURE);
            return;
        }
        ...
        if (course.strip().isBlank()) {
            ...
            Log.AddLog(sharedContext.getCurrentUserEmail(), Log.ActionName.CONTACT_STAFF, "", Log.Status.SUCCESS);
        } else {
            ...
            if (sharedContext.getCourseManager() != null) {
                ...
                while (courses.hasNext()) {
                    ...
                    if (tempCourse.hasCode(course)) {
                        ...
                        view.displaySuccess("Your inquiry has been recorded. Someone will be in touch via email soon!");
                        Log.AddLog(sharedContext.getCurrentUserEmail(), Log.ActionName.CONTACT_STAFF, course, Log.Status.SUCCESS);
                        return;
                    }
                }
            }
            ...
            Log.AddLog(sharedContext.getCurrentUserEmail(), Log.ActionName.CONTACT_STAFF, course, Log.Status.FAILURE);
        }
    }
}
